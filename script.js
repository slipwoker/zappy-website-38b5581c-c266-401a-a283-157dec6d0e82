    ...
    if (contactForm) {
        ...
        const validators = {
            ...
            textarea: (value, input) => {
                const minLength = input.getAttribute('minlength') || 10;
                if (!value.trim()) return 'This field is required';
                if (value.trim().length < minLength) return `Minimum ${minLength} characters required`;
                return null;
            },
        ...
        const validateInput = (input) => {
            ...
            if (input.hasAttribute('required') || input.required) {
                ...
                } else if (type === 'select-one' || input.tagName.toLowerCase() === 'select') {
                    error = validators.select(value);
                } else if (input.tagName.toLowerCase() === 'textarea') {
                    error = validators.textarea(value, input);
                } else {
                    error = validators.text(value, input);
                }
            ...
            if (isValid) {
                ...
                fetch(formAction, fetchOptions)
                    ...
                    .then(() => {
                        ...
                        successMsg.style.cssText = 'background: #d4edda; color: #155724; padding: 1rem; border-radius: 4px; margin-top: 1rem; text-align: center;';
                    ...
                    .catch(() => {
                        ...
                        errorMsg.style.cssText = 'background: #f8d7da; color: #721c24; padding: 1rem; border-radius: 4px; margin-top: 1rem; text-align: center;';
        ...
        contactForm.addEventListener('input', function(e) {
            ...
            if (input.hasAttribute('data-validate-on') && input.getAttribute('data-validate-on') === 'blur') {
                return;
            }
            
            if (input.classList.contains('touched') || input.getAttribute('data-touched') === 'true') {
                validateInput(input);
            }
        });
        ...
        const mutationObserver = new MutationObserver((mutations) => {
            mutations.forEach(mutation => {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                        const newAnimated = node.matches && node.matches('[data-animate], .fade-in, .animate-on-scroll') 
                            ? [node] 
                            : node.querySelectorAll ? Array.from(node.querySelectorAll('[data-animate], .fade-in, .animate-on-scroll')) : [];
                        
                        if (node.matches && node.matches('[data-animate], .fade-in, .animate-on-scroll')) {
                            newAnimated.push(node);
                        }
                        
                        newAnimated.forEach(el => {
                            if (!el.classList.contains('visible') && !el.classList.contains('animated')) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '38b5581c-c266-401a-a283-157dec6d0e82',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '38b5581c-c266-401a-a283-157dec6d0e82';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '38b5581c-c266-401a-a283-157dec6d0e82';
